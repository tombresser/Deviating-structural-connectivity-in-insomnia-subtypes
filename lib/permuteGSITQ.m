...
function [statsObj] = permuteGSITQ(edgeMat, dataVec, nPerm)
...
% output structures
estimateVec = zeros(5, length(edge_index));
tVec = zeros(nPerm+1, length(edge_index), 5);
p_lm = zeros(5, length(edge_index));
...
parfor (i = 1:nPerm, 6)
    ...
    % edges loop
    tmpT = zeros(1, length(edge_index),5); % tmp storage for loop output
    for ii = 1:length(edge_index)
        % set edge values as y
        y = perm_edges(:, edge_index(ii));
